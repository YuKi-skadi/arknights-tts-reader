...
- 生成队列持久化，关闭软件后可以继续。
- 可选“队列跑完后自动关机”（仅 Windows；本轮每个任务都尝试过后触发，部分完成或失败也会继续后续任务；暂停后继续仍可触发，停止或关闭软件不会触发；提交后延迟 60 秒，可用 `shutdown /a` 取消）。
...
3. 将剧情或自定义文本加入语音生成队列。
4. 如需无人值守运行，可勾选“队列跑完后自动关机”。
5. 生成完成后，在“朗读监听”中选择语音包。
6. 点击“选择台词区域”，框选游戏台词位置。
7. 开始游戏剧情并启动朗读监听。
